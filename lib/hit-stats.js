...
// import KeyCountStatsView from './hit-stats-view';
import fs from 'fs';
...
export default {
  ...
  isDifferentDay(prevUpdate, nextUpdate) {
    return (
      prevUpdate.getDate() !== nextUpdate.getDate() ||
      prevUpdate.getMonth() !== nextUpdate.getMonth() ||
      prevUpdate.getFullYear() !== nextUpdate.getFullYear()
    );
  },
  ...
  writeFile() {
    ...
    const lineToWrite = `${currentUpdate.toISOString()},${this.count}`;

    this.readOrCreateFile().then(() => {
